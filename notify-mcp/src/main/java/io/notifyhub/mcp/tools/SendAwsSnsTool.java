...
public class SendAwsSnsTool {

    private final NotifyHub notifyHub;

    public SendAwsSnsTool(NotifyHub notifyHub) {
    ...
    public SyncToolSpecification specification(McpJsonMapper jsonMapper) {
        String schema = """
                {
                  ...
                  "properties": {
                    "recipient": {
                      "type": "string",
                      "description": "ARN (topic or endpoint), phone number starting with '+', or alias mapped to a topic ARN"
                    },
                    "body": {
                      "type": "string",
                    ...
                    "subject": {
                      "type": "string",
                      "description": "Optional message subject (used for email subscriptions)"
                    },
                    "template": {
                      "type": "string",
        ...
        Tool tool = Tool.builder()
                .name("send_aws_sns")
                .description("Send a notification via AWS Simple Notification Service (SNS). Supports topics, platform endpoints, and SMS phone numbers.")
    ...
    private CallToolResult execute(Map<String, Object> args) {
        ...
        NotificationBuilder builder = notifyHub.to(recipient).via(Channel.AWS_SNS);
        ...
        if (subject != null && !subject.isEmpty()) {
            builder.param("subject", subject);
        }

        DeliveryReceipt receipt = builder.sendTracked();

        return ToolResultHelper.success("AWS SNS notification sent", Map.of(
                "id", receipt.getId(),
